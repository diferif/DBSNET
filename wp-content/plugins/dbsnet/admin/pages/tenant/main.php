...
jQuery(document).ready( function ($) {
	...
	// doRetrievePagination( "tenant", limit, 0, searchfor, "#plugin-data-pagination" );
...
<a href="?page=dbsnet-tenant&doaction=create-new">Create New Tenant</a>
<h3>TENANT</h3>
	...
	<div class="plugin-data-filter row">
		...
		<div class="col-sm-2">
			...
			<select id="data-limit" class="">
				<option value="3" <?php if( get_option( 'tenant_list_limit' ) == 3 ) _e( "selected='selected'" ); ?> >3</option>
				<option value="10" <?php if( get_option( 'tenant_list_limit' ) == 10 ) _e( "selected='selected'" ); ?> >10</option>
				<option value="25" <?php if( get_option( 'tenant_list_limit' ) == 25 ) _e( "selected='selected'" ); ?> >25</option>
			</select>
		</div>
	</div>
