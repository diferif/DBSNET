...
<div class="data-wrapper">
	<form class="form-horizontal" id="form-tenant">
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="text" name="tenant-username" class="form-control" id="tenant-username" required="required" value="<?php _e( $tenant->GetUsername() ) ?>" readonly="readonly">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="tenant-email">Email <strong>*</strong></label>
			<div class="col-sm-4">
				<input type="email" name="tenant-email" class="form-control" id="tenant-email" placeholder="email@email.email" required="required" value="<?php _e( $tenant->GetEmail() ) ?>">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="tenant-name">Tenant Name <strong>*</strong></label>
			<div class="col-sm-4">
				<input type="text" name="tenant-name" class="form-control" id="tenant-name" placeholder="tenant name" required="required" value="<?php _e( $tenant->GetName() ) ?>">
			</div>
		</div>
		<div class="form-group">
			...
			$.post(
				...
				function( response ) {
					...
					if( result.status ) {
						...
						//reset_form_ukm();
						location.href = "<?php echo admin_url('admin.php?page=dbsnet-tenant&doaction=edit&tenant='); ?><?php _e($tenant->GetID()); ?>&status=success";
					}else{
